...
class MY_Controller extends MX_Controller {
	...
	protected $mRedirectText;
	protected $mRedirectUrl;
    ...
    $this->uri_query 		= ($this->input->server('QUERY_STRING') != "") ? $this->uri_query = "?".$this->input->server('QUERY_STRING') : "";
    $this->mRedirectUrl = (implode("/", $this->url).$this->uri_query == "") ? $this->mModule : implode("/", $this->url).$this->uri_query;
	...
	private function _setup()
	{
		...
		$this->mPageAuth 	= ($this->session->has_userdata('page_auth')) ? $this->session->userdata('page_auth') : array();
		...
		if ( $this->ion_auth->logged_in() )
		{
			...
			if ( !empty($this->mUser) )
			{
				...
				$this->mUserMainGroupID = $this->mUserGroups[0]->id;
				$this->mUserMainGroup = $this->mUserGroups[0]->name;
				$this->mUserPanel 		= $this->mUserGroups[0]->tag_name;
				$this->mUserMainLayer = $this->mUserGroups[0]->layer_class;
			}

			if(!$this->session->has_userdata('menus')){
				$menuInline = $this->ion_auth->get_users_modules($this->mUser->id,false);
				$menus 			= $this->ion_auth->get_users_modules($this->mUser->id);
				$this->session->set_userdata(array('menus' => $menus));
				$this->session->set_userdata(array('menuInline' => $menuInline));
			}
			if(!$this->session->has_userdata('page_auth')){
				$array 				= array();
				$menus = $this->ion_auth->get_all_modules();
				foreach ($menus as $k_menu => $menu) {
					if(!isset($array[$menu->url])){
						$array[$menu->url] = array($menu->name);
					}else{
						array_push($array[$menu->url], $menu->name);
					}
				}
				$arrays = array();
			...
			if(!$this->session->has_userdata('layer_page')){
				...
				foreach ($groups as $k_group => $group) {
					$array[$group->name] = $group->layer_class;
				}
				$this->session->set_userdata(array('layer_page' => $array));
			}
		}

		$config['adminlte']['body_class'] = ($this->session->has_userdata('layer_page')) ? $this->session->userdata('layer_page') : array();
		$this->mConfig = $config;
	}
	...
	protected function verify_login($redirect_url = NULL)
	{
		if ( !$this->ion_auth->logged_in() )
		{
			$this->session->set_userdata(array('redirect' => $this->mRedirectUrl));
			$this->mRedirectText ='?redirect='.urlencode(base_url().$this->session->userdata('redirect'));
			
			if ( $redirect_url==NULL )
				$redirect_url = $this->mConfig['login_url'].$this->mRedirectText;
	...
	{
		$menuInline = $this->session->userdata('menuInline');
		if(count($menuInline) > 0){
			$myModule 	= implode("/", $this->url);
			foreach ($menuInline as $k_inline => $inline) 
				if($inline->url == $myModule) $this->mPageTitle = $inline->name;
		}
		...
		$this->mViewData['tab_title'] = empty($this->mViewData['page_title']) ? "" : $this->mPageTitlePrefix.$this->mPageTitle;
