...
class Ion_auth
{
	...
	/**
	 * caching of users and their modules
	 *
	 * @var array
	 **/
	public $_cache_user_in_module;
	...
	public function __construct()
	{
		...
		$this->_cache_user_in_module 	=& $this->ion_auth_model->_cache_user_in_module;
	 ...
	 **/
	public function in_module($check_module, $id=false, $group=false, $check_all=false){
		$this->ion_auth_model->trigger_events('in_module');
		...
		if(!$group){
			...
			if (isset($this->_cache_user_in_module[$id])){
				$modules_array = $this->_cache_user_in_module[$id];
			} else {
				$users_modules = $this->ion_auth_model->get_users_modules($id, false);
				$modules_array = array();
				foreach ($users_modules as $module){
					$modules_array[$module->id] = $module->url;
				}
				$this->_cache_user_in_module[$id] = $modules_array;
			}

			foreach ($check_module as $key => $value){
				$modules = (is_string($value)) ? $modules_array : array_keys($modules_array);

				/**
				 * if !all (default), in_array
				 * if all, !in_array
				 */
				if (in_array($value, $modules) xor $check_all){
					/**
					 * if !all (default), true
					 * if all, false
					 */
			...
			if (isset($this->_cache_user_in_module[$group])){
				$modules_array = $this->_cache_user_in_module[$group];
			} else {
				$users_modules = $this->ion_auth_model->get_groups_modules($group, false);
				$modules_array = array();
				foreach ($users_modules as $module){
					$modules_array[$module->id] = $module->url;
				}
