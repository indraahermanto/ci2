...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * caching of users and their module
	 *
	 * @var array
	 **/
	public $_cache_user_in_module = array();
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.tag_name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.layer_class')
	 ...
	 *
	 * @return array
	 * @author Indra Hermanto
	 **/
	public function get_users_modules($id=FALSE, $group=true){ // $group = grouping module into head module
		$this->trigger_events('get_users_module');

		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');
		$url = $this->uri->segment_array();
		$user_groups = $this->get_users_groups($id)->result();
		$u_groups = array();
		foreach ($user_groups as $k_groups => $groups) array_push($u_groups, $groups->id);
		$group_modules = $this->get_groups_modules($u_groups, $group);
	 ...
	 * @author Indra Hermanto
	 **/
	public function get_all_groups(){ // $group = grouping module into head module
		$this->trigger_events('get_all_groups');

		// $all_groups = array();
		$all_groups = $this->db->select($this->tables['groups'].'.name, '.$this->tables['groups'].'.layer_class')
														->get($this->tables['groups']);
	  return $all_groups->result();
	}
	 ...
	 * @author Indra Hermanto
	 **/
	public function get_all_modules(){ // $group = grouping module into head module
		$this->trigger_events('get_all_modules');

		// $all_modules = array();
		$all_modules = $this->db->select($this->tables['modules'].'.url, '.$this->tables['groups'].".name ")
														->join($this->tables['modules'], $this->tables['modules'].".id=".$this->tables['groups_modules'].".".$this->join['modules'])
														->join($this->tables['groups'], $this->tables['groups'].".id=".$this->tables['groups_modules'].".".$this->join['groups'])
														->order_by($this->tables['modules'].'.col_order, '.$this->tables['modules'].'.name, '.$this->tables['modules'].'.id')
														->get($this->tables['groups_modules']);
	  return $all_modules->result();
	}
	 ...
	 * @author Indra Hermanto
	 **/
	public function get_groups_modules($u_groups = array(), $group=true){ // $group = grouping module into head module
		$this->trigger_events('get_groups_module');
		$url = $this->uri->segment_array();
		$select_modules = $this->tables['modules'].'.id, '.$this->tables['modules'].'.url, '.$this->tables['modules'].'.name, '
										 .$this->tables['modules'].'.class, '.$this->tables['modules'].'.relation, '.$this->tables['modules'].'.stat_v';
		$where_select_modules = array($this->tables['modules'].'.status' => '1');
		$order_select_modules = $this->tables['modules'].'.col_order, '.$this->tables['modules'].'.name, '.$this->tables['modules'].'.id';
		$modules = $this->db->select($select_modules)
									->where($where_select_modules)
									->where_in($this->join['groups'], $u_groups)
									->join($this->tables['modules'], $this->tables['groups_modules'].'.'.$this->join['modules'].'='.$this->tables['modules'].'.id')
									->order_by($order_select_modules)
									->get($this->tables['groups_modules'])->result();

		$required_modules = $this->db->select($select_modules)
																 ->where($where_select_modules)
																 ->where(array($this->tables['modules'].'.required' => '1'))
																 ->order_by($order_select_modules)
																 ->get($this->tables['modules'])->result();
		$modules = array_merge($modules, $required_modules);
		// if no grouping
		if($group == false) return $modules;
		$group_modules = array();
		$temp = array();
		$na = 0;
		$u = implode("/", $url)."/";
		foreach ($modules as $k_module => $module) {
			$ar = $module; $a = $ar->relation; $b = $ar->id;
			$n = 0;
			if($ar->stat_v == 0) goto skip;
			substr_count($u, $ar->url) > 0 ? $ar->select = 'active' : $ar->select = "";
			$temp[$na][$n] = $ar;
			b:
			if($a != $b){
				$n++;
				$sub_module = $this->db->query("SELECT id, url, name, class, relation, stat_v FROM ".$this->tables['modules']." WHERE id = '".$ar->relation."'")->row();
				$ar_temp = $ar;
				$ar = $sub_module; $a = $ar->relation; $b = $ar->id;
				if($ar->stat_v == 0) goto skip;
				$ar->select = $ar_temp->select;
				$temp[$na][$n] = $ar;
				goto b;
			}
			$na++;
		...
		foreach ($temp as $k_tmp => $tmp) {
			if(count($tmp) > 1){
				krsort($tmp);
				$n = 0;
				foreach ($tmp as $k_tm => $tm) {
					if($n == 0) {
						$a = "A_".$tm->id;
						if(!isset($group_modules[$a]))
							$group_modules[$a] = $tm;
						if($tm->select == "active") $group_modules[$a]->select = $tm->select;
					}
					if($n == 1) {
						$b = $tm->id;
						if(!isset($group_modules[$a]->data[$b]))
							$group_modules[$a]->data[$b] = $tm;
						if($tm->select == "active") $group_modules[$a]->data[$b]->select = $tm->select;
					}
					if($n == 2) {
						$c = $tm->id;
						if(!isset($group_modules[$a]->data[$b]->data[$c]))
							$group_modules[$a]->data[$b]->data[$c] = $tm;
						if($tm->select == "active") $group_modules[$a]->data[$b]->data[$c]->select = $tm->select;
					}
					if($n == 3) {
						$d = $tm->id;
						if(!isset($group_modules[$a]->data[$b]->data[$c]->data[$d]))
							$group_modules[$a]->data[$b]->data[$c]->data[$d] = $tm;
						if($tm->select == "active") $group_modules[$a]->data[$b]->data[$c]->data[$d]->select = $tm->select;
					}
					$n++;
				}
			}else{
				$group_modules[$k_tmp] = $tmp[0];
			}
		}
	  return $group_modules;
	}
